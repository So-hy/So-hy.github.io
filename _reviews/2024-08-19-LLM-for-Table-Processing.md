...
이 데이터 중심의 시대에, 대량의 데이터가 표의 형태로 구조화되어 저장되고 있다. 데이터베이스 질의, 스프레드시트 조작, 웹 테이블에서의 QA Task, 이미지 테이블에서의 정보 추출 등 일상적인 작업들이 우리의 일상생활에서 흔하게 일어난다. 이러한 작업들 중 일부는 지루?하고(반복적이고) 오류가 발생하기 쉬우며, 다른 몇몇 작업들은 전문적인 기술을 필요로 하므로 더 널리 접근 가능하도록 간소화될 필요가 있다. 테이블과 관련된 작업의 자동화는 일반 대중에게 상당한 이익을 제공하며, 학계와 산업계의 큰 관심을 받고 있다고 한다.
