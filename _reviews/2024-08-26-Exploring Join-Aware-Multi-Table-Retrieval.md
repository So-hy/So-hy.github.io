...
⇒ **확장된 문제 정의**: 구체적으로, 주어진 질문 $Q$와 테이블 집합 $C$에 대해, 단일 테이블 $T_i$를 반환하는 대신, 조인 연산자가 포함된 테이블들의 랭킹 리스트 $E(Q)$를 반환하는 것이 목표이다. 여기서 $E(Q)$는 $T_{Q,1} \Join_{c1} T_{Q,2} \Join_{c2} \ldots$ 형태로 나타낼 수 있으며, T_{Q,1}, T_{Q,2}, \ldots는 C에 포함된 테이블들이고, c1, c2, \ldots는 조인 조건을 나타낸다.

<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM4NDM5MjgxNCwyOTg3NTE4NDIsNzA2MT
