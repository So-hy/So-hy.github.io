...
첫 번째 어텐션 메커니즘(디코더 이력에 대한 어텐션)은 $\hat{y}_t$와 이전에 생성된 모든 토큰 시퀀스 $Y_t$ 사이의 관련성을 계산한다:
	
$$a_Y = \text{Attention}(Q = \hat{y}t, K = Y_t, V = Y_t)$$

이 과정을 통해 현재 단계에서 이전 단계의 정보 중 어떤 부분에 집중할지를 결정한다.

두 번째 어텐션은 현재 토큰이 전체 입력 데이터, 즉 DSL과 질문 표현  $H_{DSL} \| H$(인코더로 부터의 입력 + 디코더의 입력) 에 얼마나 집중해야 하는지를 결정한다.
...
이렇게 생성된 두 어텐션 결과  $a_Y$ 와  $a_{\text{seq}}$ 를 결합하여 현재 단계의 컨텍스트 표현  h_c 를 생성합니다.
	•	이 단계에서는 Layernorm을 사용하여 표현을 정규화하고, LSTM의 출력과 어텐션 결과를 결합합니다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc1MTA1ODQxNiw2NjQ1OTMyMzAsNTU5MT
M4NTQyLC0yMjgwODU3MDEsLTExNDAxNDMyOTAsLTE1NTAyOTU5
ODIsLTk0NjE0MjMwNCwtMTM2NzUxOTI0NiwtOTg0NTY1NjMxLD
ExOTQxMTI4OTksODg5NzIzNTcsLTMyMDM4ODU3MiwtMTk2MDc2
MDQzMywtNDgwMjU1MDU1LDk3ODY1ODksMTgzNDQ1MjMwNCw1Nz
I4NTg4NzQsNjIyMzM2MzkwLC0xNDUyODU2NDg4LDUxMjY5MTAz
NF19
-->