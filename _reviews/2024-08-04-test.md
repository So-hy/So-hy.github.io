...
$$
\text{sim}(H_Q, H_D) = \sum_{i \in H_Q} \max_{j \in H_D} \frac{H_Q^i \cdot H_D^j}{\|H_Q^i\| \|H_D^j\|}
$$
...
각 토큰에 대해서
$$
L(Q, D^+, D^−) = \sum_{i \in D^+} \sum_{j \in D^-} \log\left(1 + \frac{\exp\left(\text{sim}(H_Q, H_D^-^j)\right)}{\exp\left(\text{sim}(H_Q, H_D^+^i)\right)}\right)
$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4NzYwNTYzMjcsNjIyMzM2MzkwLC0xND
UyODU2NDg4LDUxMjY5MTAzNCwxODgzNTU2Njc1LC0xMzgzODA4
OTQ1LDU4MTU0MjksNTA0ODI2NDAyLC04OTQwMDY5OCwtODkyNT
