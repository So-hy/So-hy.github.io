...
CTRLSciTab은 주로 **과학 논문**에서 **숫자 추론**과 관련된 테이블-설명 쌍을 통합한 데이터셋이다. 이를 위해 **arXiv.org**에서 테이블 메타데이터를 사용해 원본 과학 논문을 검색하고, 해당 테이블과 관련된 문장을 추출하여 **도메인 특화 지식** 기반을 구성했습니다.

-   테이블 설명에 언급된 **하이라이트된 셀**을 추가하여, 사용자 선호를 반영하는 프롬프트로 사용합니다.
-   문서들은 XML 형식으로 변환되고, **탐욕 알고리즘**을 사용하여 테이블과 문장(엔티티 참조 문장)을 일치시킵니다.
-   **중복 제거** 및 **엔티티 감지**를 통해 불필요한 정보는 배제하고, 테이블 설명과 일치하는 핵심 엔티티만을 유지합니다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0NzgxOTg3OTQsLTQ4ODMxNzQzMSwxMT
