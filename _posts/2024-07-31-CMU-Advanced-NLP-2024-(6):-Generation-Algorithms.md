...
빔서치는 가장 유명한 디코딩 메서드 중 하나이다. 여러 후보를 유지하며 검색을 진행하며, 각 단계마다 가장 확률이 높은 몇 개의 후보를 선택하여 다음 단계로 넘어간다. 그리디 디코딩보다 더 나은 결과를 제공하지만, 후보수가 많아지면 계산 비용이 증가한다는 단점이 있다. 이는 BFS(Breadth-first search,너비우선탐색)의 한 종류이며, 각 탐색 단계에서 다양한 가능성 중 일부만을 선택하여 계속 탐색을 진행한다. 이 알고리즘은 ‘빔 너비’ 또는 ‘빔 크기’라는 매개변수로 제한된 수의 후보를 유지하며, 이 크기가 결과의 질과 계산 속도에 큰 영향을 미친다. 각 시간 단계마다 여러 옵션을 고려하고, 전체 시퀀스를 한 번에 생성하는 대신 점진적으로 시퀀스를 완성해 나가는 방식으로, 효율적이고 실용적인 탐색을 가능하게 한다. 이는 특히 긴 시퀀스나 복잡한 해를 찾는 상황에서 유리하며, 빔 서치는 최적의 해에 접근하기 위해 계속해서 다양한 옵션들을 평가하는 과정을 반복한다. 이를 그림으로 나타내면 아래와 같다.
...
결과적으로 보았을 때, 항상 최고의 결과를 내는 것은 아니지만(중간에 가지치기 당),
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTIwMDM4NDgyLDE0Nzc2MTc1NzcsLTk3Nz
I3NDE0NSwtOTg2NTgwODM4LDM4NDI5MTM0MiwtMTU5MzE2OTAy
MiwtMzY3NTc1MjA5LC0xNTM0MjMwMTU1LC05OTEzODIwNTQsLT
g3MDM0MzUzOSw2ODAyMzA0NzEsODY0ODQ2NzQzLDEwNzAxMDg5
MTIsLTIwMjE2Njg3NiwxMTk2NDM1ODkyLDEyNTM4OTY5OTksLT
Y5OTQwMjg5MiwtMTA5MzMyNTMxOCwxMDYxMTcyNTM0LDE2NDkx
NDIxODBdfQ==
-->