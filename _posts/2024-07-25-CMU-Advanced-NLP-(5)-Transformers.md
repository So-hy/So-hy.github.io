...
이렇게 분할된 각각의 헤드에 대해 어텐션 메커니즘을 실행한다. 이 어텐션 메커니즘은 한번에 병렬적으로 처리하게 된다. 그리고 결과를 출력하게 된다. 당연히 결과의 열은 Q의 열과 같게 된다. (쿼리에 대해 계산하기 때문..) 그리고 아까 쪼겠던 것들을 다시 연결함으로써 최종적으로 결과가 도출된다. 이렇게 분할해서 멀티 어텐션을 수행함으로써 더
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTA5Nzc2Njk1MiwtMTIxMDcwMzM4NiwtNj
