...
이 챕터에선 파인 튜닝, 그 중 대규모 모델에 대한 파인 튜닝에 대해 이야기한다. 파인 튜닝의 전체적인 개념은 비교적 간단하다. 언어 모델을 계속 훈련시키면서 원하는 데이터에 맞추는 것이다. 하지만 이러한 훈련 방식은 메모리를 많이 차지하며, 다른 대안에 비해 상대적으로 불안정할 수 있다.
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDgwOTE3ODQzLDExNjI3Njg2MTksLTU0MD
