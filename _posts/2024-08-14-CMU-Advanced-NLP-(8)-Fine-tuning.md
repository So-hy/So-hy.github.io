...
문제를 어렵게 만드는 요인은 매우 다양하기 때문에 이를 모두 포괄하기는 어렵다. 또한 주제, 언어, 말하는 스타일 등의 관점에서 분포 내/외인지 여부도 고려해야 한다. 이는 디버깅 및 평가 강의에서 다룰 주제 중 하나이지만, 교수가 좋아하는 방식은 데이터를 세분화하여 세그먼트별로 성능 차이를 살펴보는 것이라고 설명하였다. 세그먼트로 나눌 수 있는 모든 것은 난이도를 결정하는 요소이기 때문에 이에 대해 많은 이야기를 할 수 있습니다.
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM3MTg5MjY1OSwxOTU4NzM4MjAwLDE3MT
