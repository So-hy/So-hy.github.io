...
다른 방법으로는 Adapters 가 있다. 어댑터는 표준 트랜스포머 아키텍처(혹인 Pre-trained Model) 내에 추가적인 레이를 삽입해 훈련한다. 이 레이어는 주어진 입력을 다운스케일한 후, 다시 업스케일하여 원래의 입력과 결합합니다. 이렇게 하면 훈련해야 할 파라미터의 수가 줄어들어 메모리 사용량이 줄어듭니다.
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTUyMTU1NjY3MiwtMTc0MjE5ODExNSwtMz
